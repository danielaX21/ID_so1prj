        ...
        if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0) {
            ...
            if (stat(file_path, &file_info) < 0) {
                ...
                if (n < 0) {
                    ...
                    write_snapshot(file, buffer); // Scrierea datelor în fișier
                }
            }
        }
    }
...
void traverse_directory(const char* directory_name, int level, int fd) {
    DIR *dir;
    struct dirent *entry;
    struct stat file_info;
    ...
    create_snapshot(directory_name, fd);
        ...
        if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0) {
            ...
            if (stat(file_path, &file_info) < 0) {
                ...
                if (S_ISDIR(file_info.st_mode)) {
                    traverse_directory(file_path, level + 1, fd);
                } else {
